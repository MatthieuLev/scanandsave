<template>
  <b-form class="login-form">
    ...
    <p class="message forgotPassword">
      <a v-on:click="$emit('changeCurrentPage','forgotPassword')">
        Mot de passe oublié ?
      </a>
    </p>
    ...
    <p class="message">Pas encore enregistré ?
      <a v-on:click="$emit('changeCurrentPage','registration')">
        Créer un compte
      </a>
    </p>
  ...
  export default {
    ...
    methods: {
      async signIn(e) {
        ...
        this.errorPassword = '';
        this.errorMessage = '';
        const result = await database.signIn(this.form.email, this.form.password);
        if (!result.message) {
          this.$router.push('/');
        } else {
          switch (result.code) {
